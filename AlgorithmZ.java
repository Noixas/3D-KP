public class AlgorithmZ extends Algorithm
{
  ...
  public void updateEP(SolutionSet placedBoxes, List<Vector3D> EP, Box newBox)
  {
    ...
    for(int i = 0; i<placedBoxes.getSize();i++)
    {
      Box aBox = placedBoxes.get(i);
      Vector3D aSize = a.getSize();
  ...
  {
    if(axis.x != 0 && newBox.x <= prevBox.x == false)//Can't be proyected into x so return
    return false;
    else if(axis.y != 0 && newBox.y <= prevBox.y == false)//Can't be proyected into y so return
    return false;
    else if(axis.z != 0 && newBox.z <= prevBox.z == false)//Can't be proyected into z so return
    return false;
    else
    return true;
  }
